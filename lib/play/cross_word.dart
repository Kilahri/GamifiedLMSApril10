...
// ============================================================================
// CROSSWORD-SPECIFIC ACHIEVEMENT + MISSION MODELS
// ============================================================================

enum CrossAchievementTier { bronze, silver, gold }

class CrossAchievement {
  final String id;
  final String title;
  final String description;
  final String emoji;
  final CrossAchievementTier tier;
  final Color color;
  final bool Function(Map<String, dynamic> stats) isUnlocked;

  const CrossAchievement({
    required this.id,
    required this.title,
...
// ============================================================================
// CROSSWORD ACHIEVEMENTS (8 total — bronze / silver / gold tiers)
// ============================================================================

const List<CrossAchievement> kCrossAchievements = [
  CrossAchievement(
    id: 'cross_first_word',
    title: 'First Word',
    description: 'Solve your first crossword word',
    emoji: '📝',
    tier: CrossAchievementTier.bronze,
    color: Color(0xFF9C27B0),
    isUnlocked: _crossFirstWord,
  ),
  CrossAchievement(
    id: 'cross_5_words',
  ...
  CrossAchievement(
    ...
    emoji: '🎯',
    tier: CrossAchievementTier.gold,
    color: Color(0xFFFFC107),
    isUnlocked: _crossNoHints,
  ),
  CrossAchievement(
    id: 'cross_all_words',
    title: 'Completionist',
  ...
  CrossAchievement(
    ...
    emoji: '⏱️',
    tier: CrossAchievementTier.silver,
    color: Color(0xFF4CAF50),
    isUnlocked: _crossSwift,
  ),
  CrossAchievement(
  ...
  CrossAchievement(
    ...
    isUnlocked: _crossNoWrong,
  ),
];

bool _crossFirstWord(Map<String, dynamic> s) =>
    (s['sessionWordsSolved'] ?? 0) >= 1;
bool _cross5Words(Map<String, dynamic> s) =>
    (s['sessionWordsSolved'] ?? 0) >= 5;
bool _crossNoHints(Map<String, dynamic> s) =>
    (s['hintsUsed'] ?? 1) == 0 && (s['puzzlesCompleted'] ?? 0) >= 1;
bool _crossAllWords(Map<String, dynamic> s) =>
    (s['fullPuzzlesCompleted'] ?? 0) >= 1;
bool _crossSwift(Map<String, dynamic> s) =>
    (s['timeRemainingOnWin'] ?? 0) >= 120;
bool _crossHardComplete(Map<String, dynamic> s) =>
    (s['hardPuzzlesCompleted'] ?? 0) >= 1;
bool _crossAllTopics(Map<String, dynamic> s) =>
    (s['topicsCompleted'] ?? 0) >= 5;
bool _crossNoWrong(Map<String, dynamic> s) =>
    (s['sessionPerfectWords'] ?? 0) >= 5;

// ============================================================================
// CROSSWORD MISSIONS (5 per session)
// ============================================================================

const List<CrossMission> kCrossMissions = [
  CrossMission(
    id: 'cross_m1',
  ...
  CrossMission(
    id: 'cross_m2',
    description: 'Solve a word with no wrong attempts',
    target: 1,
...
// ============================================================================
// CROSSWORD ACHIEVEMENT SERVICE
// ============================================================================

class CrossAchievementService {
  Future<List<CrossAchievement>> checkAchievements({
    ...
    final doc = FirebaseFirestore.instance
        .collection('crossword_achievements')
        .doc(userId);
    final snap = await doc.get();
    final existing = List<String>.from(snap.data()?['unlocked'] ?? []);

    final newlyUnlocked = <CrossAchievement>[];
    for (final ach in kCrossAchievements) {
...
// ============================================================================
// MISSIONS PANEL WIDGET (Crossword-specific, collapsible)
// ============================================================================

class _CrossMissionsPanel extends StatefulWidget {
  final Map<String, dynamic> sessionStats;
  final Color accentColor;

  const _CrossMissionsPanel({
    required this.sessionStats,
    this.accentColor = const Color(0xFF7B4DFF),
  });

  @override
  State<_CrossMissionsPanel> createState() => _CrossMissionsPanelState();
}

class _CrossMissionsPanelState extends State<_CrossMissionsPanel> {
  bool _expanded = true;

  @override
  Widget build(BuildContext context) {
    final done =
        kCrossMissions.where((m) {
        ...
        children: [
          ...
          if (_expanded)
            Padding(
              ...
              child: Column(
                children: kCrossMissions.map((m) => _missionRow(m)).toList(),
              ),
            ),
        ],
      ),
  ...
  Widget _missionRow(CrossMission mission) {
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              color: isDone ? _C.green.withOpacity(0.2) : _C.w08,
              border: Border.all(color: isDone ? _C.green : _C.w38, width: 1),
            ),
            child:
                isDone
                    ? const Icon(Icons.check, color: _C.green, size: 11)
                    : null,
          ),
          ...
          Text(
            ...
            style: const TextStyle(color: _C.amber, fontSize: 9),
          ),
        ],
      ),
    );
...
class _ScienceCrosswordScreenState extends State<ScienceCrosswordScreen>
  ...
  final CrossAchievementService _achService = CrossAchievementService();
  ...
  Map<String, dynamic> _sessionStats = {
    ...
    'sessionPerfectWords': 0,
    'sessionDownSolved': 0,
    'sessionAcrossSolved': 0,
    'sessionCompletedWithHints': 0,
  };
  ...
  void _checkAnswer() {
    ...
    if (input == pw.word) {
      // ── CORRECT ──────────────────────────────────────────────────────────
      AudioService.playSoundEffect('tile.wav');
      ...
      final hadNoWrong =
          (_wordWrongAttempts[_activeWordIdx.toString()] ?? 0) == 0;

      setState(() {
        _score += pts;
        for (final k in cells) _flashCells[k] = _C.green;

        _sessionStats['sessionWordsSolved'] =
            (_sessionStats['sessionWordsSolved'] as int) + 1;

        if (pw.dir == 'across') {
          _sessionStats['sessionAcrossSolved'] =
              (_sessionStats['sessionAcrossSolved'] as int) + 1;
        } else {
          _sessionStats['sessionDownSolved'] =
              (_sessionStats['sessionDownSolved'] as int) + 1;
        }

        if (hadNoWrong) {
          _sessionStats['sessionPerfectWords'] =
              (_sessionStats['sessionPerfectWords'] as int) + 1;
        }
      });
      ...
      // ── WRONG ────────────────────────────────────────────────────────────
      AudioService.playSoundEffect('error.wav');
      setState(() {
        _wrongAttempts++;
        _wordWrongAttempts[_activeWordIdx.toString()] =
            (_wordWrongAttempts[_activeWordIdx.toString()] ?? 0) + 1;
      });
  ...
  void _finishGame(bool completed) async {
    ...
    final allSolved = _solvedIndices.length == _puzzleWords.length;
    _sessionStats['sessionCompletedWithHints'] =
        (allSolved && _hintsLeft > 0) ? 1 : 0;
    _sessionStats['fullPuzzlesCompleted'] = allSolved ? 1 : 0;
    _sessionStats['timeRemainingOnWin'] = allSolved ? _timeLeft : 0;
    _sessionStats['hardPuzzlesCompleted'] =
        (allSolved && _selectedDiff == 'hard') ? 1 : 0;
    _sessionStats['hintsUsed'] = _hintsUsed;
    _sessionStats['puzzlesCompleted'] = allSolved ? 1 : 0;

    // Load cumulative topic completions
    int topicsCompleted = 0;
    try {
      final prefs = await SharedPreferences.getInstance();
      final Set<String> completedTopics = Set<String>.from(
        jsonDecode(prefs.getString('cross_topics_completed') ?? '[]') as List,
      );
      if (allSolved) completedTopics.add(_selectedTopic!.id);
      await prefs.setString(
        'cross_topics_completed',
        jsonEncode(completedTopics.toList()),
      );
      topicsCompleted = completedTopics.length;
    } catch (_) {}
    _sessionStats['topicsCompleted'] = topicsCompleted;
    ...
    try {
      final newAchs = await GameAchievementService.checkNewAchievements(
        ...
        gameId: GameAchievementService.GAME_CROSSWORD,
        stats: _sessionStats,
      );
      if (newAchs.isNotEmpty && mounted) {
        _showAchievementDialog(newAchs);
      }
    } catch (e) {
  ...
  void _showAchievementDialog(List<GameAchievement> achievements) {
    showDialog(
      context: context,
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: bg,
                  border: Border.all(
                    color: isActive ? _C.gridLineActive : _C.gridLine,
