...
class _ReadScreenState extends State<ReadScreen> {
  ...
  void initState() {
    super.initState();
    // Clear stale in-memory state from any previous account session
    resetReadingState();
    _loadBooks();
    ...
    return GestureDetector(
      onTap: () async {
        final result = await Navigator.push(
          ...
          MaterialPageRoute(
            builder:
                (ctx) => BookReaderScreen(book: book, userId: widget.userId),
          ),
        );
        if (result == true) {
          _loadTotalPointsFromFirestore();
...
class _BookReaderScreenState extends State<BookReaderScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    // Apply quiz bonus points for matching topic (only once per book)
  ...
  Future<void> _applyQuizBonus() async {
    ...
    try {
      ...
      // Check bonus flag from reading_progress doc
      final progressDoc =
          await FirebaseFirestore.instance
              .collection('reading_progress')
              .doc(widget.userId)
  ...
  Future<void> _saveProgress() async {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .set({
            widget.book.title: {
              'completedChapters': readingProgress[widget.book.title]!.toList(),
