...
class _WatchScreenState extends State<WatchScreen>
  ...
  // Load lessons
  // ─────────────────────────────────────────────
  Future<void> _loadLessonsFromStorage({bool forceRefresh = false}) async {
    ...
    try {
      ...
      debugPrint('WatchScreen load error: $e');
  ...
  void _markLessonComplete() {
    ...
    _saveProgress();
    widget.onLessonCompleted?.call(currentLessonIndex, 20);
  ...
  void _navigateToQuiz() async {
    ...
    if (result != null && result is int) {
      ...
      widget.onLessonCompleted?.call(currentLessonIndex, result);
    }
  }
                          ...
                          children: [
                            ...
                            Expanded(
                              child: SingleChildScrollView(
                                // ← ADD THIS
                                reverse:
                                    true, // ← keeps cursor visible when keyboard opens
                                child: Column(
                                  children: [
                                    TextField(
                                      controller: _notesController,
                                      maxLines:
                                          12, // ← fixed lines instead of expands
                                      style: const TextStyle(
                                        color: Colors.white,
                                        fontSize: 15,
                                      ),
                                    ...
                                    // ← extra padding so save button isn't hidden by keyboard
                                    SizedBox(
                                      height:
                                          MediaQuery.of(
                                            context,
